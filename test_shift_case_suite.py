...
# --- 1. API del ciclo ---
for fn in ["fun handleShiftTap()", "fun toSentenceCase(", "fun nextCase(", "enum class CaseState"]:
    check(f"EditEngine implementa {fn}", fn in ee, f"falta {fn}")
check("Orden del ciclo LOWER/SENTENCE/UPPER", all(s in ee for s in ["LOWER", "SENTENCE", "UPPER"]))
check("Sentence case: primera letra (D-M1)", "indexOfFirst" in ee and "uppercaseChar" in ee)
...
check("Aviso Primera mayúscula/Sentence case", '"Primera mayúscula"' in ee and '"Sentence case"' in ee)
