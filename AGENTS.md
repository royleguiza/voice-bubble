...
- [ ] Hito 4 – Pegado inteligente (Accessibility) → **HÍBRIDO aprobado 2026-09-05** (`plan-hito4-burbuja-hibrida.md`, D-HB0…D-HB7): descongelamiento parcial acotado a inyección puntual burbuja (`ACTION_SET_TEXT` + fallback `PASTE`) con tercer teclado; teclado propio sigue prioritario vía `commitText`; prohibido monitoreo continuo, prohibido `Log` con contenido, prohibido inyectar en contraseñas/`FLAG_SECURE`; switch propio `kb_bubble_a11y_inject_enabled` OFF por defecto
