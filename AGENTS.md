...
**Deuda reportada en dispositivo (2026-09-10, r127; (1) RESUELTA local 2026-09-22, (2)(3) APARCADAS por el dueño)**: (1) clipboard no guardaba imágenes aun con el switch ON — causa: `hasMimeType("image/*")` es igualdad exacta y jamás igualaba image/png (rama muerta); fix con `compareMimeTypes` + barrido de todos los items + firma anti-duplicados + aviso en primer plano, suite clipboard 37/37; (2) capa snippets del teclado perdió minimalismo: al abrirla, shift (izq. de Z) y borrar quedan bajos vs resto, y la selección en edición hoy rellena en verde/azul completo en vez del borde punteado azul por tarjeta de antes — solo tener en cuenta; (3) editando un snippet no andan gestos del teclado (deslizar borrando, deslizar en barra espaciadora).
